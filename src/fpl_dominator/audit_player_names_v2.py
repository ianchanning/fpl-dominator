...
def get_sanitized_lastname(name: str) -> str:
    """
    Extracts and sanitizes the last word of a name string.
    This is the core of our new, more intelligent matching.
    """
    last_name = name.split(' ')[-1]
    return sanitize_name(last_name)

def audit_player_name_resolution_v2():
    print("--- PLAYER NAME RESOLUTION AUDIT (V2 - LAST NAME CONCORDANCE) ---")
    ...
    # --- Prepare for Join ---
    players_df['match_key_full'] = players_df['Surname'].apply(sanitize_name)
    players_df['match_key_last'] = players_df['Surname'].apply(get_sanitized_lastname)
    players_df['Team_Full_For_Join'] = players_df['Team'].map(TEAM_SHORT_TO_FULL)

    success_count, collision_count, no_match_count = 0, 0, 0

    print("\n--- BEGINNING ENTITY RESOLUTION AUDIT (V2) ---")
    ...
    for _, row in set_pieces_df.iterrows():
        ...
        for duty_type, takers in duties.items():
            for taker_name in takers:
                ...
                if not taker_name or taker_name.lower() == 'nan': continue

                sanitized_taker_full = sanitize_name(taker_name)
                sanitized_taker_last = get_sanitized_lastname(taker_name)
                
                # Perform the match using the new, superior logic
                matches = players_df[
                    (players_df['Team_Full_For_Join'] == club_full_name) &
                    (
                        (players_df['match_key_full'] == sanitized_taker_full) | # First, try a perfect full name match
                        (players_df['match_key_last'] == sanitized_taker_last)   # Then, fall back to last name concordance
                    )
                ]
    ...
    if collision_count == 0:
        print(">>> VERDICT: The matching LOGIC is sound. Remaining 'No Match' errors are confirmed DATA GAPS.")
    else:
        print(">>> VERDICT: CRITICAL COLLISIONS DETECTED. Manual intervention required.")

if __name__ == "__main__":
    audit_player_name_resolution_v2()

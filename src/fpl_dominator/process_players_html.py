    ...
    for row in rows:
        ...
        if info_button:
            ...
            if "chance of playing" in aria_label:
                ...
                "injury" in aria_label
                or "suspended" in aria_label
                or "permanently" in aria_label
            ):
                status = "INJURY"
    ...
    if os.path.exists(squad_html):
        ...
        if not squad_df.empty:
            ...
            if market_dfs:
                ...
                price_map = all_market.set_index(["Surname", "Position"])[
                    "Price"
                ].to_dict()
            ...
            print(
                f"    - SUCCESS: {squad_csv} forged with {len(squad_df)} squad members."
            )
        else:

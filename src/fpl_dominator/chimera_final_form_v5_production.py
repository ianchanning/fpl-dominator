...
def enrich_with_set_pieces(players_df, set_piece_path, score_model):
    ...
    if not os.path.exists(set_piece_path):
        print(
            f"!!! CRITICAL FAILURE: Set-piece database not found at '{set_piece_path}'. Aborting enrichment."
        )
        return players_df
...
def forge_final_form_squad(gameweek_dir: str):
    ...
    if not os.path.exists(OMNISCIENT_DB_PATH):
        print(
            f"!!! CRITICAL FAILURE: Omniscient Database not found at '{OMNISCIENT_DB_PATH}'. Aborting."
        )
        return False
    ...
    starter_final_score = pulp.lpSum(
        [players.loc[i, "Final_Score"] * starter_vars[i] for i in players.index]
    )
    bench_cost_penalty = pulp.lpSum(
        [
            (squad_vars[i] - starter_vars[i]) * players.loc[i, "Price"] * THRIFT_FACTOR
    ...
    prob += (
        pulp.lpSum([players.loc[i, "Price"] * squad_vars[i] for i in players.index])
        <= 100.0,
        "Cost",
    )
        ...
        prob += (
            pulp.lpSum(
                [
                    squad_vars[i]
                    for i in players.index
                    if players.loc[i, "Team_TLA"] == team
                ]
            )
            <= 3,

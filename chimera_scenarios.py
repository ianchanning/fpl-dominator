...
def print_squad_details(squad, starters, bench, status, scenario_name):
    ...
    if status == 'Optimal':
        ...
        print(starters[['Surname', 'Team', 'Position', 'Price', 'TP']].sort_values(by=['Position', 'TP'], ascending=[True, False]).to_string(index=False))
        
        print("\n--- BENCH (RUTHLESSLY Cost-Optimized) ---")
        print(bench[['Surname', 'Team', 'Position', 'Price', 'TP']].sort_values(by=['Position', 'Price']).to_string(index=False))
...
def forge_squad(players_df, scenario_name, include_players=[], exclude_players=[], max_cost=100.0):
    """
    ...
    prob = pulp.LpProblem(f"FPL_Scenario_{scenario_name.replace(' ', '_')}", pulp.LpMaximize)
    ...
    starter_points = pulp.lpSum([players_df.loc[i, 'TP'] * starter_vars[i] for i in players_df.index])
...
if __name__ == "__main__":
    ...
    if not os.path.exists(ENRICHED_DB_PATH):
        print(f"!!! CRITICAL FAILURE: Database not found at '{ENRICHED_DB_PATH}'. Aborting.")
    else:
        ...
        if 'M.Salah' in players['Surname'].values:
            forge_squad(players, "'Gods of Chaos'", include_players=['Haaland', 'M.Salah'])
        else:
